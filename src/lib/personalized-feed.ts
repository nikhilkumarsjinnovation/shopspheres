...
export interface FeedResponse {
  ...
  userPersona: string;
  summary: string;
  carousels: FeedCarousel[];
  totalLiveProducts: number;
}

export async function fetchFeedFromDB(userId: string | null): Promise<FeedResponse> {
  ...
  carousels.push({
    ...
    badge: 'Bestsellers',
    products: [...allApprovedProducts].sort((a, b) => (b.average_rating || 0) - (a.average_rating || 0)).slice(0, 8),
  });
